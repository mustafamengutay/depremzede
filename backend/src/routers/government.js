...
router.get('/inventory', async (req, res) => {
    try {
        ...
        console.error(e);  
        res.status(500).send({ error: 'Internal Server Error', details: e.message });  // Hata mesajını yanıta ekleyin
    }
});
...
router.patch('/inventory/:id', async (req, res) => {
    ...
    try {
        const inventoryItem = await Envanter.findByIdAndUpdate(req.params.id, req.body, { new: true, runValidators: true });
